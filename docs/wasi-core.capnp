...
interface WasiCore {
  ...
  argsGet @0 (
    argvPtr :Pointer $type("char **"), # A pointer to a buffer to write the argument pointers.
    argvBuf :Text $type("char *") # A pointer to a buffer to write the argument string data.
  ) -> (
    error :ErrnoT
  );
  ...
  argsSizesGet @1 (
    argc :Pointer $type("size_t *"), # The number of arguments.
    argv_buf_size :Pointer $type("size_t *") # The size of the argument string data.
  ) -> (
    error :ErrnoT
  );
  ...
  clockTimeGet @3 (
    ...
    time :Pointer $type("TimestampT") # The time value of the clock.
  ) -> (
    error :ErrnoT
  );
  ...
  environGet @4 (
    environ :Pointer $type("char **"), # A pointer to a buffer to write the environment variable pointers.
    environ_buf :Text $type("char *")# A pointer to a buffer to write the environment variable string data.
  ) -> (
    error :ErrnoT
  );
  ...
  # Return command-line argument data sizes.
  environSizesGet @5 (
    environ_count :Pointer $type("size_t *"), # The number of environment variables.
    environ_buf_size :Pointer $type("size_t *")# The size of the environment variable string data.
  ) -> (
    error :ErrnoT
  );
  ...
  fdPwrite @19 (
    ...
    iovs :List(CiovecT), # List of scatter/gather vectors from which to retrieve data.
  ...
  fdRead @20 (
    fd :FdT, # The file descriptor from which to read data.
    iovs :Pointer $type("iovec_t *"), # List of scatter/gather vectors to which to store data.
    iovs_len :SizeT,
    nread :Pointer $type("size_t") # The number of bytes read.
  ) -> (
    error :ErrnoT
  );
  ...
  fdReaddir @21 (
    ...
    buf :Text, # The buffer where directory entries are stored.
    buf_len :SizeT, #
    cookie :DircookieT, # The location within the directory to start reading. 
    bufused :Pointer $type("SizeT") # The number of bytes stored in the read buffer. If less than the size of the read buffer, the end of the directory has been reached.
  ) -> (
    error :ErrnoT
  );
  ...
  fdSeek @23 (
    ...
    newoffset :Pointer $type("size_t") # The new offset of the file descriptor, relative to the start of the file.
  ) -> (
    error :ErrnoT
  );
  ...
  fdTell @25 (
    fd :FdT, # The file descriptor to inspect.
    offset :Pointer $type("FilesizeT") # The current offset of the file descriptor, relative to the start of the file.
  ) -> (
    error :ErrnoT
  );
  ...
  fdWrite @26 (
    ...
    nwritten :Pointer $type("SizeT") # The number of bytes written.
  ) -> (
    error :ErrnoT
  );
  ...
  pathOpen @31 (
    ...
    fd :Pointer $type("FdT") # The file descriptor of the file that has been opened.
  ) -> (
    error :ErrnoT
  );
  ...
  pathReadlink @32 (
    ...
    path_len :SizeT,
    buf :Pointer $type("Text"), # The buffer to which to write the contents of the symbolic link.
    bufused :Pointer $type("SizeT") # The number of bytes placed in the buffer.
  ) -> (
    error :ErrnoT
  );
  ...
  sockRecv @42 (
    ...
    ri_data :Pointer $type("IovecT"), # List of scatter/gather vectors to which to store data.
    ri_flags :RiflagsT, # Message flags.
    ro_datalen :Pointer $type("SizeT"), # Number of bytes stored in ri_data.
    ro_flags :Pointer $type("RoFlags") # Message flags.
  ) -> (
    error :ErrnoT 
  );
  ...
  sockSend @43 (
    ...
    si_data :Pointer $type("ciovec_t *"), # List of scatter/gather vectors to which to retrieve data
    si_data_len :SizeT, 
    si_flags :SiflagsT, # Message flags.
    so_datalen :Pointer $type("SizeT") # Number of bytes transmitted.
  ) -> (
    error :ErrnoT    
  );
